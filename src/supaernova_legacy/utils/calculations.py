...
def compute_sigma_ae_time(
    ...
    relative = True
    dm = (
        np.max(mask, axis=-1) == 1.0
    )  # keep all spectra that have any portion unmasked (=1)
    dtbin = 0.1
    tmax = 1.0
    tmin = 0.0
    ...
    for ibin in np.unique(bins):
        ...
        if not relative:
            ...
            sigma_t[:, ibin] = np.sqrt(
                np.sum((msei) ** 2 * mask, axis=0) / np.sum(mask, axis=0)
            )
    ...
    return (
        ...
        t_bin_edge.astype(np.float32),
        t_bin_cent.astype(np.float32),
    )
