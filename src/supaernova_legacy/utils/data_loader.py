...
def load_data(
    ...
    scale_sigma_observed=True,
    set_data_min_val=0.0,
    print_params=False,
    npz=False,
    to_tensor=False,
):
    ...
    if to_tensor:
        ...
        for k, v in data.items():
            print(k, v.dtype)
            v_dtype = v.dtype
            if v_dtype in {np.float32, np.float64}:
...
def split_train_and_val(train_data, params):
    ...
    for k, v in train_data.items():
        ...
        if v.shape[0] == num_samples:
            ...
            train_data[k] = v[
                :-num_val_samples
            ]  # remove samples now in val set from train set
        else:
            val_data[k] = v
...
def get_train_mask(data, params):
    ...
    if params["twins_cut"]:
        _in_twins, dm_twins = get_twins_mask(data)
        dm &= dm_twins
...
def get_twins_mask(data):
    ...
    set_snf = set(data["names"])
    set_boone = set(df_twins["name"])
