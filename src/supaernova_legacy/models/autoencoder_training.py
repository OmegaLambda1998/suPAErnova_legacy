...
def train_step(
    ...
    if model.params["vary_mask"]:
        ...
        for i in range(mask_vary.shape[0]):
            ...
            mask_vary[i : i + 1, :nii] = mask_vary[
                i : i + 1, np.random.rand(nii).argsort()
            ]
    ...
    if epoch == 0:
        print(
            "Number of training SN to use for training: ", np.sum(train_data["mask_sn"])
        )
...
def train_model(train_data, val_data, test_data, model):
    ...
    for epoch in range(model.params["epochs"]):
        ...
        _training_loss, training_loss_terms = train_step(
            model,
            optimizer,
        ...
        if epoch % model.params["val_every"] == 0:
            ...
            # Calculate test loss
            _test_loss, test_loss_terms = test_step(model, test_data)
