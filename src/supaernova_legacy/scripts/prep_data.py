#!/usr/bin/env python
"""This code prepares data for the Autoencoder model."""
...
def parse_arguments(inputs: "Sequence[str] | None") -> argparse.Namespace:
    ...
    parser.add_argument("--config", default="data", type=str)

    return parser.parse_args(inputs)


def prep_data(inputs: "Sequence[str] | None" = None) -> dict[str, "Any"]:
    ...
    data_params = YParams(
        os.path.abspath(args.yaml_config), args.config, print_params=True
    )

    # Used cached result if it exists.
    savepath = Path(data_params["savepath"])
